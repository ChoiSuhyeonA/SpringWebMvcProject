...
public class BoardMapperTest {
	...
	public void insertTest() {
		...
		for(int i=1; i<=320; i++) {
			...
			article.setContent("테스트 중이니까 방해하지 마세요~" + i);
...
//		}
	...
	//게시물 수정 테스트:
	//VO의 setter를 사용하여 수정 내용 (글 제목, 글 내용)을 입력하고
	//수정을 테스트 해 보세요.
	@Test
	public void updateTest() {
		BoardVO article = new BoardVO();
		article.setBoardNo(1);
		article.setTitle("수정된 제목입니다.");
		article.setContent("수정 테스트입니다. 메롱메롱~");
		mapper.update(article);
		System.out.println("수정 후 정보: " + mapper.getArticle(1));
	}
	
	
	//게시물 삭제 테스트: 게시글 번호를 통한 삭제를 확인하세요. (44)
	@Test
